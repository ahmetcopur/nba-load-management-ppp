...
out=REPO_ROOT/'results/portable_identification_stage2/player_linear_trends_sparse_exact.csv';pd.DataFrame(rows).to_csv(out,index=False);print(pd.DataFrame(rows).to_string(index=False))
