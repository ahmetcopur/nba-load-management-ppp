...
SRC='/mnt/data/nba_continue/run_nonlinear_multinomial_stage.py'
spec=importlib.util.spec_from_file_location('m',SRC);m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
OUT=Path('/mnt/data/nba_continue/nonlinear_multinomial_stage_v1_final');OUT.mkdir(parents=True,exist_ok=True)
...
lpm=pd.read_csv('/mnt/data/nba_continue/cadence_harmonized_results/canonical_vs_cadence_model_comparison.csv').to_dict('records')
...
zp=Path('/mnt/data/nba_nonlinear_multinomial_robustness_v1.zip')
