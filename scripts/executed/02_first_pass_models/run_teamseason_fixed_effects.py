...
out=Path('/mnt/data/_model_run');pd.DataFrame(rows).to_csv(out/'teamseason_fe_focal.csv',index=False);pd.concat(tabs).to_csv(out/'teamseason_fe_coefficients.csv',index=False)
